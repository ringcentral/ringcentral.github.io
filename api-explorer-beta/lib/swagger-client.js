    ...
    if(contentType) {
      if(contentType.indexOf("application/json") === 0 || contentType.indexOf("+json") > 0) {
        try {
          ...
          // ===== BEGIN JCW OAUTH EDIT RESPONSE =====
          if ( out.obj["access_token"].length>0 ) {
            ac = out.obj["access_token"];
            $('#input_apiKey').val(ac);
            window.authorizations.add("key", new ApiKeyAuthorization("Authorization", "Bearer " + ac, "header"));
          }
          // ===== END JCW OAUTH EDIT RESPONSE =====
        } catch (ex) {
          // do not set out.obj
